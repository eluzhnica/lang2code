...
from utils import ProperLSTM, BottleEmbedding, BottleLSTM
...
class Encoder(nn.Module):
    ...
    def forward(self, batch):
        ...
        src = Variable(batch['src'], requires_grad=False)
        ...
        varTypes = Variable(batch['varTypes'], requires_grad=False)
        ...
        var_context = var_context.view(var_context.size(0), -1,
                                       var_context.size(3))  # interleave type and name, type first

        methodCamel = Variable(batch['methodNames'].transpose(1, 2).contiguous(), requires_grad=False)
        ...
        method_context = method_context.view(method_context.size(0), -1,
                                             method_context.size(3))  # interleave type and name, type first
