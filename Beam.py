...
class TreeBeam(object):
    def __init__(self, size, cuda, vocabs, rnn_size):
        ...
        self.finished = []
        self.stacks = [[('MemberDeclaration', '<s>',
                         Variable(self.tt.FloatTensor(1, 1, self.rnn_size).zero_(), requires_grad=False))] for i in
                       range(0, self.size)]  # stacks for non terminals
    ...
    def getCurrentState(self):
        ...
        for i in range(0, len(self.nextYs[-1])):
            ...
            if len(self.stacks[i]) == 0:
                (nt, parent_rule, parent_state) = ('MemberDeclaration', '<s>',
                                                   Variable(self.tt.FloatTensor(1, 1, self.rnn_size).zero_(),
                                                            requires_grad=False))
            else:
