...
def shiftLeft(t, pad):
    ...
    padding = torch.zeros(t.size(0), 1).fill_(pad).long()
...
class ProperLSTM(nn.LSTM):
    ...
    def forward(self, seq, seq_lens):
        ...
        new_unsorted_states[0] = new_unsorted_states[0] * non_zero_length_mask.view(1, -1, 1).expand_as(
            new_unsorted_states[0])
        new_unsorted_states[1] = new_unsorted_states[1] * non_zero_length_mask.view(1, -1, 1).expand_as(
            new_unsorted_states[1])
