        ...
        if char > 0:
            ...
            try:
                ret = ([self.stoi['<s>']] if self.start and start else []) + [
                    self.stoi[w] if w in self.stoi else self.stoi['<unk>'] for w in words] + (
...
class Dataset:
    def compute_batches(self, batch_size, vocabs, max_camel, rank, num_gpus, decoder_type, randomize=True, trunc=-1,
        ...
        for i in range(rank, len(self.examples), num_gpus):
            if not no_filter and len(self.examples[i]['next_rules']) > 200:
                continue
            total += 1
...
class CDDataset(Dataset):
            ...
            if test or (len(js['seq2seq']) <= opt.src_seq_length and len(js['code']) <= opt.tgt_seq_length):
                ...
                self.examples.append(
                    {'src': src,
                     ...
                     'seq2seq_vocab': Vocab(js['seq2seq_nop'], 0, 100000000, start=False, stop=False),
                     # A vocab just for this sentence
                     'children': children,

                     'concode': [j for i in zip(js['varTypes'], js['varNames']) for j in i] + [j for i in
                                                                                               zip(js['methodReturns'],
                                                                                                   js['methodNames'])
                                                                                               for j in i],
                     'concode_vocab': Vocab(
                         js['varNames'] + js['varTypes'] + js['methodReturns'] + js['methodNames'] + [
                             'concode_copy_placeholder'], 0, 1000000, start=False, stop=False),
    ...
    def toNumbers(self, vocabs):
        for e in self.examples:
            ...
            e['seq2seq_in_src_nums'] = e['seq2seq_vocab'].to_num(
                vocabs['seq2seq'].addStartOrEnd(e['seq2seq']))  # use the local vocab for this sentence
            e['code_in_src_nums'] = e['seq2seq_vocab'].to_num(
                vocabs['code'].addStartOrEnd(e['code']))  # use the local vocab for this sentence
            # For concode decoder--------------
            ...
            e['method_in_src_nums'] = e['concode_vocab'].to_num(
                e['concode_method'])  # use the local vocab for this sentence
            # -------------------------------------------------------
            ...
            e['methodNames_nums'] = vocabs['names_combined'].to_num([(split_camel_case(w)) for w in e['methodNames']],
                                                                    char=1)
    ...
    def outputStats(self, vocabs):
        ...
        print('Average Code Characters: ' + str(
            sum([len(' '.join(e['code'])) for e in self.examples]) * 1.0 / len(self.examples)))
        ...
        print('Average AST Nodes: ' + str(
            sum([len(rhs(r).split('___')) for e in self.examples for r in e['next_rules']]) * 1.0 / len(self.examples)))
        print('Max AST Nodes: ' + str(max([len(rhs(r).split('___')) for e in self.examples for r in e['next_rules']])))
        print('Percent getters: ' + str(
            sum([int(isGetter(e['origcode'])) for e in self.examples]) * 1.0 / len(self.examples)))
        print('Percent setters: ' + str(
            sum([int(isSetter(e['origcode'])) for e in self.examples]) * 1.0 / len(self.examples)))
        ...
        def match_source(src, code, names):
            for w in src:
                if (w not in vocabs['code'].stoi or vocabs['code'].stoi[w] == vocabs['code'].stoi[
                    '<unk>']) and w in code and w not in names:
                    return True
            return False

        src_copies = np.mean([1 if match_source(e['src'], e['origcode'],
                                                e['varNames'] + e['varTypes'] + e['methodReturns'] + e[
                                                    'methodNames']) else 0 for e in self.examples]) * 100.0

        def match_type(type_list, code):
            for typ in type_list:
                if typ in code and (
                        typ not in vocabs['code'].stoi or vocabs['code'].stoi[typ] == vocabs['code'].stoi['<unk>']):
                    return True
            return False

        type_copies = np.mean(
            [1 if match_type(e['methodReturns'] + e['varTypes'], e['origcode']) else 0 for e in self.examples]) * 100.0

        print(
            'Number of variable copies: {}, function copies: {}, source copies: {}, Type copies: {} '.format(var_copies,
                                                                                                             fn_copies,
                                                                                                             src_copies,
                                                                                                             type_copies))
    ...
    def make_batch_into_tensor(batch, vocabs, max_camel):
        ...
        torch_batch['src_in_src'] = make_batch_elem_into_tensor(batch, 'src_in_src_nums',
                                                                batch[0]['concode_vocab'].stoi['<blank>'])
        torch_batch['var_in_src'] = make_batch_elem_into_tensor(batch, 'var_in_src_nums',
                                                                batch[0]['concode_vocab'].stoi['<blank>'])
        torch_batch['method_in_src'] = make_batch_elem_into_tensor(batch, 'method_in_src_nums',
                                                                   batch[0]['concode_vocab'].stoi['<blank>'])
        torch_batch['concode_src_map_methods'] = expandBatchOneHot(torch_batch['method_in_src'],
                                                                   batch[0]['concode_vocab'].stoi['<blank>'],
                                                                   width=max_local_vocab_in_batch)
        torch_batch['concode_src_map_vars'] = expandBatchOneHot(torch_batch['var_in_src'],
                                                                batch[0]['concode_vocab'].stoi['<blank>'],
                                                                width=max_local_vocab_in_batch)
        torch_batch['concode_vocab'] = [b['concode_vocab'] for b in batch]  # Store this for replace unk
        torch_batch['concode_next_rules_in_src_nums'] = make_batch_elem_into_tensor(batch,
                                                                                    'concode_next_rules_in_src_nums',
                                                                                    local_vocab_blank)
        ...
        torch_batch['seq2seq_copy'] = CDDataset.stack_with_padding([torch.LongTensor(b['seq2seq_copy']) for b in batch],
                                                                   0, start_symbol=True, stop_symbol=True)
...
if __name__ == "__main__":
    ...
    vocabs = {
        ...
        'nt': Vocab([w for e in train.examples for w in e['nt']], 0, 10000, start=False, stop=False, pad=True,
                    unk=True),
        'seq2seq': Vocab([w for e in train.examples for w in e['seq2seq']], opt.seq2seq_words_min_frequency, 45000,
    ...
    vocabs['prev_rules'] = Vocab(
        ...
        0, 10000000, stop=False, pad=True, unk=True)
