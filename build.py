...
def split_types(type_name):
  final_types = type_name.replace("<", "REPLACETYPETOKEN")\
    .replace(">", "REPLACETYPETOKEN")\
    .replace(",", "REPLACETYPETOKEN")\
    .replace(".", "REPLACETYPETOKEN")
  final_types = final_types.split("REPLACETYPETOKEN")
  return final_types
  ...
  for line in open(fname, 'r'):
    ...
    for methodName in js["memberFunctions"]:
      for methodInstance in js["memberFunctions"][methodName]:
        ...
        # methodReturns.append("None" if split_types(methodInstance[0]) is None else split_types(methodInstance[0])) # The first element is the return type
        ...
        # memberVarTypes = [split_types(value.encode('ascii', 'replace').decode()) for key, value in
        #                   js["memberVariables"].items()]
